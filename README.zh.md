...
> [!IMPORTANT]
> **Open Codex Mobile 是实验性的 Android-first 移动端控制层，不是托管式 AI Agent 服务。**
>
...
- ✍️ **手机继续对话**：在手机上发送消息，Bridge 会转发到电脑端 `codex app-server` 继续同一个 thread。
...
- 🎛️ **运行参数控制**：暴露 model、reasoning、access mode、fast mode，但最终仍以电脑端 provider 支持为准。
...
- 🖥️ 手机可以发起工作，但能跑什么由电脑端 Codex runtime 和 sandbox/provider 决定。
- 🎛️ 手机上不支持的模型或 fast mode 选择，应该视为 provider no-op 或电脑端报错，而不是手机端能力。
...
- 🛡️ 权限模式会传给桌面 runtime，但具体行为依赖 Codex app-server 版本和 sandbox 兼容性。
