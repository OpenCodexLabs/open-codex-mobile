...
internal suspend fun CodexService.performSecureHandshake() {
    ...
    val savedLastSeq = snapshot.relayLastAppliedBridgeOutboundSeq?.toIntOrNull() ?: 0
    ...
    // Trusted reconnect may attach after a bridge restart whose replay counter starts fresh.
    val resumeLastSeq =
        if (handshakeMode == CodexSecureHandshakeMode.trustedReconnect) {
            0
        } else {
            savedLastSeq
        }
    ...
    secureSession =
        CodexSecureSession(
            ...
            lastInboundBridgeOutboundSeq = resumeLastSeq,
    ...
    val resume =
        SecureResumeState(
            sessionId = sessionId,
            keyEpoch = serverHello.keyEpoch,
            lastAppliedBridgeOutboundSeq = resumeLastSeq,
        )
