...
internal class IncomingEventRouter(
    ...
    private val remapPinnedThreadIdsFromServer: (List<String>) -> List<String> = { it },
    /**
    ...
    fun dispatchNotification(
        ...
        when (m) {
            ...
            "thread/pin/updated" -> handleThreadPinUpdated(obj)
            "thread/pins/updated" -> handleThreadPinsUpdated(obj)
    ...
    private fun handleThreadPinUpdated(params: Map<String, JSONValue>?) {
        if (params == null) return
        val threadId = extractThreadId(params) ?: return
        val event = envelopeEventObject(params)
        val pinned =
            params["pinned"]?.boolValue
                ?: params["isPinned"]?.boolValue
                ?: event?.get("pinned")?.boolValue
                ?: event?.get("isPinned")?.boolValue
                ?: return
        val pinnedRank =
            params["pinnedRank"]?.intValue
                ?: params["pinned_rank"]?.intValue
                ?: params["pinnedIndex"]?.intValue
                ?: params["pinned_index"]?.intValue
                ?: event?.get("pinnedRank")?.intValue
                ?: event?.get("pinnedIndex")?.intValue
        val list = threads.value
        val idx = list.indexOfFirst { it.id == threadId }
        if (idx >= 0) {
            threads.value =
                    ...
                    list.mapIndexed { i, t ->
                        if (i == idx) {
                            t.copy(
                                isPinned = pinned,
                                hasPinnedState = true,
                                pinnedRank = if (pinned) pinnedRank else null,
                            )
                        } else {
                            t
                        }
    ...
    private fun handleThreadPinsUpdated(params: Map<String, JSONValue>?) {
        if (params == null) return
        val event = envelopeEventObject(params)
        val pinnedThreadIds =
            firstStringArray(params, listOf("pinnedThreadIds", "pinned_thread_ids"))
                ?: event?.let { firstStringArray(it, listOf("pinnedThreadIds", "pinned_thread_ids")) }
                ?: return
        val effectivePinnedThreadIds = remapPinnedThreadIdsFromServer(pinnedThreadIds)
        val ranks = effectivePinnedThreadIds.mapIndexed { index, id -> id to index }.toMap()
        threads.value =
            sortThreadsForSidebar(
    ...
    private fun firstStringArray(
        ...
        for (k in keys) {
            val items =
                obj[k]
                    ?.arrayValue
                    ?.mapNotNull { it.stringValue?.trim()?.takeIf { value -> value.isNotEmpty() } }
            if (items != null) {
                return items
            }
        }
