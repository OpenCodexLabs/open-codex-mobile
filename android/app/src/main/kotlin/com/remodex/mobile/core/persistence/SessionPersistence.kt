...
class SessionPersistence(
    ...
    private fun encodeCachedThreadEntry(
        ...
        val fields =
            listOf(
                ...
                if (thread.isPinned) "1" else "0",
                if (thread.hasPinnedState) "1" else "0",
                thread.pinnedRank?.toString().orEmpty(),
            )
    ...
    private fun decodeCachedThreadEntry(raw: String): CachedThreadEntry? {
        ...
        val thread =
            CodexThread(
                ...
                isPinned = parts.getOrNull(16).boolCacheFlag(),
                hasPinnedState = parts.getOrNull(17).boolCacheFlag(),
                pinnedRank = parts.getOrNull(18)?.toIntOrNull(),
            )
    ...
    private fun String?.boolCacheFlag(): Boolean =
        this?.trim() == "1"
