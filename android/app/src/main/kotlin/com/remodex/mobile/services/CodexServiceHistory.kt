...
internal suspend fun CodexService.syncThreadHistoryInternal(
    ...
    val hasLocalTimeline = messageTimelineStore.messagesByThread.value[tid].orEmpty().isNotEmpty()
    if (shouldSkipThreadHistoryHydration(force, hydratedThreadIds.contains(tid), hasLocalTimeline)) return
...
internal fun shouldSkipThreadHistoryHydration(
    force: Boolean,
    alreadyHydrated: Boolean,
    hasLocalTimeline: Boolean,
): Boolean = !force && alreadyHydrated && hasLocalTimeline
...
internal suspend fun CodexService.catchUpThreadAfterSelectionOrReconnect(threadId: String) {
    ...
    if (refreshed && isRunning) {
        ...
        val hasLocalTimeline = messageTimelineStore.messagesByThread.value[tid].orEmpty().isNotEmpty()
        runCatching { syncThreadHistoryInternal(tid, force = !hasLocalTimeline) }
    }
}
