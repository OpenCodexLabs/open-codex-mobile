...
object SidebarThreadGrouping {
    ...
    fun makeGroups(threads: List<CodexThread>): List<SidebarThreadGroup> {
        val pinnedGroup = makePinnedGroup(threads)
        ...
        return listOf(pinnedGroup) + projectGroups + listOfNotNull(chatsGroup, archivedGroup)
    }
    ...
    fun applyPinnedOrder(
        groups: List<SidebarThreadGroup>,
        pinnedThreadIds: List<String>,
    ): List<SidebarThreadGroup> {
        if (pinnedThreadIds.isEmpty()) return groups
        val ranks = pinnedThreadIds.mapIndexed { index, id -> id to index }.toMap()
        return groups.map { group ->
            if (group.kind != SidebarThreadGroupKind.Pinned) {
                group
            } else {
                val reordered =
                    group.threads.sortedWith { lhs, rhs ->
                        val lr = ranks[lhs.id] ?: Int.MAX_VALUE
                        val rr = ranks[rhs.id] ?: Int.MAX_VALUE
                        when {
                            lr != rr -> lr.compareTo(rr)
                            else -> lhs.id.compareTo(rhs.id)
                        }
                    }
                group.copy(threads = reordered)
            }
        }
    }
    ...
    private fun makePinnedGroup(threads: List<CodexThread>): SidebarThreadGroup {
        val pinnedThreads =
            sortPinnedThreads(
                threads.filter { thread ->
                    thread.syncState != CodexThreadSyncState.archivedLocal && thread.isPinned
                },
            )
        val first = pinnedThreads.firstOrNull()
        val firstLive = threads.firstOrNull { it.syncState != CodexThreadSyncState.archivedLocal }
        return SidebarThreadGroup(
            id = "pinned",
            label = "Pinned",
            kind = SidebarThreadGroupKind.Pinned,
            sortDate = first?.updatedAt ?: first?.createdAt ?: firstLive?.updatedAt ?: firstLive?.createdAt ?: Instant.EPOCH,
            projectPath = null,
    ...
    private fun sortPinnedThreads(threads: List<CodexThread>): List<CodexThread> =
        threads.sortedWith { lhs, rhs ->
            val lr = lhs.pinnedRank ?: Int.MAX_VALUE
            val rr = rhs.pinnedRank ?: Int.MAX_VALUE
            when {
                lr != rr -> lr.compareTo(rr)
                else -> {
                    val l = lhs.updatedAt ?: lhs.createdAt ?: Instant.EPOCH
