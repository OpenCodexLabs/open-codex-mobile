...
test("threadPinSet writes desktop pinned state", (t) => {
  ...
  fs.writeFileSync(statePath, JSON.stringify({
    ...
    "electron-persisted-atom-state": {
      "pinned-thread-ids": ["thread-c"],
    },
    unrelated: true,
  }));

  const pinResult = __test.threadPinSet({
    threadId: "thread-c",
    pinned: true,
  });
  assert.deepEqual(pinResult.pinnedThreadIds, ["thread-c", "thread-a", "thread-b"]);
  assert.deepEqual(pinResult.previousPinnedThreadIds, ["thread-a", "thread-b", "thread-c"]);
  assert.equal(pinResult.pinnedRank, 0);

  let state = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.deepEqual(state["pinned-thread-ids"], ["thread-c", "thread-a", "thread-b"]);
  assert.deepEqual(
    state["electron-persisted-atom-state"]["pinned-thread-ids"],
    ["thread-c", "thread-a", "thread-b"],
  );
  assert.equal(state.unrelated, true);

  const unpinResult = __test.threadPinSet({
    thread_id: "thread-a",
    pinned: false,
  });
  assert.deepEqual(unpinResult.pinnedThreadIds, ["thread-c", "thread-b"]);
  assert.deepEqual(unpinResult.previousPinnedThreadIds, ["thread-c", "thread-a", "thread-b"]);
  assert.equal(unpinResult.pinnedRank, null);

  state = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.deepEqual(state["pinned-thread-ids"], ["thread-c", "thread-b"]);
  assert.deepEqual(
    state["electron-persisted-atom-state"]["pinned-thread-ids"],
    ["thread-c", "thread-b"],
  );
...
test("threadPinsSet reorders desktop pinned state and preserves unknown existing pins", (t) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remodex-global-state-"));
  const statePath = path.join(dir, "global-state.json");
  const previousStatePath = process.env.REMODEX_CODEX_GLOBAL_STATE_PATH;
  process.env.REMODEX_CODEX_GLOBAL_STATE_PATH = statePath;
  ...
  const result = __test.threadPinsSet({
    pinnedThreadIds: ["thread-b", "thread-a", "thread-b"],
  });

  assert.deepEqual(result.pinnedThreadIds, ["thread-b", "thread-a", "thread-c"]);
  assert.deepEqual(result.previousPinnedThreadIds, ["thread-a", "thread-b", "thread-c"]);
  const state = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.deepEqual(state["pinned-thread-ids"], ["thread-b", "thread-a", "thread-c"]);
});
