...
const previousDeviceStateDir = process.env.REMODEX_DEVICE_STATE_DIR;
const secureTransportStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "remodex-secure-transport-"));
process.env.REMODEX_DEVICE_STATE_DIR = secureTransportStateDir;

after(() => {
  if (previousDeviceStateDir === undefined) {
    delete process.env.REMODEX_DEVICE_STATE_DIR;
  } else {
    process.env.REMODEX_DEVICE_STATE_DIR = previousDeviceStateDir;
  }
