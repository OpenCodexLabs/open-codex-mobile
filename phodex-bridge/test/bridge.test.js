...
test("normalizeRelayBoundJsonRpcMessage keeps allowed untracked method-bearing responses", () => {
  const normalized = normalizeRelayBoundJsonRpcMessage(JSON.stringify({
    id: "req-thread-list",
    method: "thread/list",
    payload: {
    ...
    allowedUntrackedResponseMethods: new Set(["thread/list"]),
  });
...
test("inferRelayBoundResponseMethodForDesktopState detects thread list responses", () => {
  assert.equal(inferRelayBoundResponseMethodForDesktopState(JSON.stringify({
    id: "req-thread-list",
    result: {
      data: [
        {
...
test("injectDesktopThreadStateForRelay marks desktop pinned threads", (t) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remodex-global-state-"));
  const statePath = path.join(dir, "global-state.json");
  const previousStatePath = process.env.REMODEX_CODEX_GLOBAL_STATE_PATH;
  process.env.REMODEX_CODEX_GLOBAL_STATE_PATH = statePath;
  t.after(() => {
    if (previousStatePath === undefined) {
      delete process.env.REMODEX_CODEX_GLOBAL_STATE_PATH;
    } else {
      process.env.REMODEX_CODEX_GLOBAL_STATE_PATH = previousStatePath;
    }
    fs.rmSync(dir, { recursive: true, force: true });
  });
  fs.writeFileSync(statePath, JSON.stringify({
    "pinned-thread-ids": ["thread-top-level"],
    "electron-persisted-atom-state": {
      "pinned-thread-ids": ["thread-pinned"],
    },
  }));

  const injected = JSON.parse(injectDesktopThreadStateForRelay(JSON.stringify({
    id: "req-thread-list",
    result: {
      data: [
        { id: "thread-normal", title: "Normal", cwd: "/tmp/normal" },
        { id: "thread-pinned", title: "Pinned", cwd: "/tmp/pinned" },
        { id: "thread-top-level", title: "Top-level pinned", cwd: "/tmp/top-level" },
      ],
    },
  }), "thread/list"));

  assert.equal(injected.result.data[0].isPinned, undefined);
  assert.equal(injected.result.data[1].isPinned, true);
  assert.equal(injected.result.data[1].pinned, true);
  assert.equal(injected.result.data[1].pinnedRank, 1);
  assert.equal(injected.result.data[2].isPinned, true);
  assert.equal(injected.result.data[2].pinned, true);
  assert.equal(injected.result.data[2].pinnedRank, 0);
});

test("injectDesktopThreadStateForRelay appends missing desktop pinned threads on terminal pages", (t) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remodex-pinned-sessions-"));
  const codexHome = path.join(dir, "codex-home");
  const statePath = path.join(dir, "global-state.json");
  const sessionsDir = path.join(codexHome, "sessions", "2026", "05", "20");
  const previousStatePath = process.env.REMODEX_CODEX_GLOBAL_STATE_PATH;
  ...
  fs.writeFileSync(statePath, JSON.stringify({
    "pinned-thread-ids": ["thread-existing", "thread-missing"],
  }));
  fs.writeFileSync(path.join(codexHome, "session_index.jsonl"), [
    JSON.stringify({
      id: "thread-missing",
      thread_name: "Pinned from desktop",
      updated_at: "2026-05-20T12:00:00Z",
    }),
    "",
  ].join("\n"));
  fs.writeFileSync(
    path.join(sessionsDir, "rollout-2026-05-20T12-00-00-thread-missing.jsonl"),
    [
      JSON.stringify({
  ...
  const injected = JSON.parse(injectDesktopThreadStateForRelay(JSON.stringify({
    ...
    result: {
      data: [
        { id: "thread-existing", title: "Existing", cwd: "/tmp/existing" },
      ],
      nextCursor: null,
    },
  }), "thread/list"));

  assert.equal(injected.result.data.length, 2);
  assert.equal(injected.result.data[0].isPinned, true);
  assert.deepEqual(injected.result.data[1], {
    id: "thread-missing",
    title: "Pinned from desktop",
    name: "Pinned from desktop",
    preview: "Pinned preview",
    createdAt: "2026-05-20T11:58:00Z",
...
test("mobile pinned override reasserts stale desktop writebacks", () => {
  const override = createMobilePinnedOverride({
    previousPinnedThreadIds: ["desktop-a", "desktop-b"],
    pinnedThreadIds: ["phone-b", "phone-a"],
  });

  assert.equal(
    resolveMobilePinnedOverrideChange({
      pinnedThreadIds: ["desktop-a", "desktop-b"],
      override,
    }).action,
    "reassert-mobile",
  );
...
test("mobile pinned override accepts a new desktop order", () => {
  const override = createMobilePinnedOverride({
    previousPinnedThreadIds: ["desktop-a", "desktop-b"],
    pinnedThreadIds: ["phone-b", "phone-a"],
  });
  const resolution = resolveMobilePinnedOverrideChange({
    pinnedThreadIds: ["desktop-c", "desktop-a"],
    override,
  });

  assert.equal(resolution.action, "accept-desktop");
  assert.equal(resolution.nextOverride, null);
  assert.deepEqual(resolution.pinnedThreadIds, ["desktop-c", "desktop-a"]);
});

test("mobile pinned override supports empty pinned state", () => {
  const override = createMobilePinnedOverride({
    previousPinnedThreadIds: ["desktop-a"],
    pinnedThreadIds: [],
  });
  const resolution = resolveMobilePinnedOverrideChange({
    pinnedThreadIds: ["desktop-a"],
    override,
  });

  assert.equal(resolution.action, "reassert-mobile");
  assert.deepEqual(resolution.pinnedThreadIds, []);
});
