...
test("resolveBridgeRelaySession creates and stores a persistent relay session", () => {
...
test("resolveBridgeRelaySession reuses a saved relay session", () => {
  const state = makeDeviceState({
  ...
  assert.equal(resolved.isPersistent, true);
  assert.equal(resolved.sessionId, "relay-session-1");
...
test("resolveBridgeRelaySession persists the relay session id until reset", () => {
  withTempDeviceStateEnv(() => {
    ...
    assert.equal(resolved.isPersistent, true);
    assert.equal(reloaded.relaySessionId, resolved.sessionId);
