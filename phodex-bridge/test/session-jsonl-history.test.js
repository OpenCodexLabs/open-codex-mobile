...
test("readThreadReadResultFromSessionJsonl builds a chronological thread/read fallback", () => {
  const filePath = "/tmp/thread-read.jsonl";
  const lines = [
    {
      timestamp: "2026-05-05T23:31:11.000Z",
      type: "session_meta",
      payload: {
        id: "thread-read-jsonl",
      },
    },
  ...
  const result = readThreadReadResultFromSessionJsonl(filePath, {
    threadId: "thread-read-jsonl",
    limit: 20,
    fsModule: {
      readFileSync: () => lines.map((line) => JSON.stringify(line)).join("\n"),
    },
  ...
  assert.equal(result.remodexJsonlFallback, true);
  assert.equal(result.hasMore, false);
  assert.equal(result.thread.id, "thread-read-jsonl");
  assert.equal(result.thread.turns.length, 5);
  assert.deepEqual(
