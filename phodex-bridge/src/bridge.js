...
const {
  handleGitRequest,
  writePinnedThreadIdsToCodexGlobalState,
} = require("./git-handler");
...
const {
  readThreadReadResultFromSessionJsonl,
...
const DEFAULT_CODEX_GLOBAL_STATE_PATH = path.join(os.homedir(), ".codex", ".codex-global-state.json");
...
function startBridge({
  ...
  let mobilePinnedOverride = null;
  const pinnedStateWatcher = createCodexPinnedStateWatcher({
    onChange: handleCodexPinnedStateChanged,
  });
  ...
  function handleApplicationMessage(rawMessage) {
    ...
    if (handleGitRequest(rawMessage, sendApplicationResponse, {
      ...
      onThreadPinSet: sendThreadPinUpdatedNotification,
      onThreadPinsSet: sendPinnedThreadsSetNotification,
    })) {
      return;
    }
    ...
    if (handleBridgeManagedThreadReadRequest(rawMessage, sendApplicationResponse)) {
      return;
    }
  ...
  // Mirrors accepted local pin updates back to the phone and nudges Desktop when refresh is enabled.
  function sendThreadPinUpdatedNotification(result) {
    const threadId = readString(result?.threadId || result?.thread_id);
    const pinned = typeof result?.pinned === "boolean"
      ? result.pinned
      : typeof result?.isPinned === "boolean"
        ? result.isPinned
        : null;
    if (!threadId || pinned == null) {
      return;
    }
    rememberMobilePinnedOverride({
      pinnedThreadIds: result?.pinnedThreadIds,
      previousPinnedThreadIds: result?.previousPinnedThreadIds || result?.previous_pinned_thread_ids,
    });

    sendApplicationResponse(JSON.stringify({
      method: "thread/pin/updated",
      params: {
        threadId,
        thread_id: threadId,
        pinned,
        isPinned: pinned,
        pinnedRank: result?.pinnedRank ?? null,
        pinnedIndex: result?.pinnedIndex ?? null,
      },
    }));
    sendPinnedThreadsUpdatedNotification(result?.pinnedThreadIds);
    desktopRefresher.queueRefresh("phone", {}, "phone thread/pin/set");
  }

  function sendPinnedThreadsUpdatedNotification(pinnedThreadIds) {
    if (!Array.isArray(pinnedThreadIds)) {
      return;
    }
    sendApplicationResponse(JSON.stringify({
      method: "thread/pins/updated",
      params: {
        pinnedThreadIds,
        pinned_thread_ids: pinnedThreadIds,
      },
    }));
  }

  function sendPinnedThreadsSetNotification(result) {
    const pinnedThreadIds = result?.pinnedThreadIds || result?.pinned_thread_ids;
    rememberMobilePinnedOverride({
      pinnedThreadIds,
      previousPinnedThreadIds: result?.previousPinnedThreadIds || result?.previous_pinned_thread_ids,
    });
    sendPinnedThreadsUpdatedNotification(pinnedThreadIds);
    desktopRefresher.queueRefresh("phone", {}, "phone thread/pins/set");
  }

  function rememberMobilePinnedOverride({ pinnedThreadIds, previousPinnedThreadIds }) {
    if (!Array.isArray(pinnedThreadIds)) {
      return;
    }
    mobilePinnedOverride = createMobilePinnedOverride({
      pinnedThreadIds,
      previousPinnedThreadIds,
    });
  }

  function handleCodexPinnedStateChanged(pinnedThreadIds) {
    const resolution = resolveMobilePinnedOverrideChange({
      pinnedThreadIds,
      override: mobilePinnedOverride,
    });
    mobilePinnedOverride = resolution.nextOverride;
    if (resolution.action === "reassert-mobile") {
      try {
        writePinnedThreadIdsToCodexGlobalState(resolution.pinnedThreadIds);
      } catch {
        mobilePinnedOverride = null;
        sendPinnedThreadsUpdatedNotification(normalizePinnedThreadIds(pinnedThreadIds));
        return;
      }
    }
    sendPinnedThreadsUpdatedNotification(resolution.pinnedThreadIds);
  }
  ...
  function handleBridgeManagedThreadReadRequest(rawMessage, sendResponse = sendApplicationResponse) {
    const response = maybeBuildJsonlThreadReadResponse(rawMessage);
    if (!response) {
      return false;
    }

    rememberThreadFromMessage("phone", rawMessage);
    relaySanitizedResponseMethodsById.set(String(response.id), {
      method: "thread/read",
      createdAt: Date.now(),
    });
  ...
  function maybeBuildJsonlThreadReadResponse(rawMessage) {
    const request = parseJsonlThreadReadRequest(rawMessage);
    if (!request) {
      return null;
    }
    ...
    try {
      ...
      const result = readThreadReadResultFromSessionJsonl(rolloutPath, {
        threadId,
        limit: params.limit,
        maxLimit: RELAY_TURNS_LIST_MAX_INITIAL_LIMIT,
        cursor: params.cursor || params.turnCursor || params.beforeCursor,
      });
      if (!result?.thread?.turns?.length) {
        return null;
      }
      ...
      console.warn(`[remodex] thread/read jsonl response failed: ${error.message}`);
      return null;
    }
  }
  ...
  function sanitizeRelayBoundCodexMessage(rawMessage) {
    ...
    const normalizedMessage = normalizeRelayBoundJsonRpcMessage(rawMessage, {
      ...
      allowedUntrackedResponseMethods: relaySanitizedRequestMethods,
    });
    ...
    const inferredMethod = trackedRequest?.method || inferRelayBoundResponseMethodForDesktopState(normalizedMessage);
    if (!inferredMethod) {
    ...
    const withDesktopState = injectDesktopThreadStateForRelay(normalizedMessage, inferredMethod);
    return sanitizeThreadHistoryImagesForRelay(withDesktopState, inferredMethod);
  }
...
function parseJsonlThreadReadRequest(rawMessage) {
  const parsed = parseBridgeJSON(rawMessage);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return null;
  }
  ...
  const includeTurns = params.includeTurns ?? params.include_turns;
  if (includeTurns === false) {
    return null;
  }

  if (hasRelayCursor(params.cursor) || hasRelayCursor(params.turnCursor) || hasRelayCursor(params.beforeCursor)) {
    return null;
  }
...
function normalizeRelayBoundJsonRpcMessage(rawMessage, {
  ...
  allowedUntrackedResponseMethods = null,
} = {}) {
  const parsed = parseBridgeJSON(rawMessage);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return null;
  ...
  if (hasMethod && hasResponseId && !isRelayBoundServerRequestMethod(parsed.method)) {
    ...
    const isAllowedUntrackedResponse = allowedUntrackedResponseMethods?.has(parsed.method)
      && (hasResult || hasError || hasPayload)
      && !Object.prototype.hasOwnProperty.call(parsed, "params");
    if (isTrackedResponse || isAllowedUntrackedResponse) {
...
function inferRelayBoundResponseMethodForDesktopState(rawMessage) {
  const parsed = parseBridgeJSON(rawMessage);
  if (looksLikeThreadListResponse(parsed)) {
    return "thread/list";
  }
  return "";
}

function looksLikeThreadListResponse(parsed) {
  const result = parsed?.result;
  if (!result || typeof result !== "object" || Array.isArray(result)) {
    return false;
  }

  const threadsKey = findThreadListResultKey(result);
  if (!threadsKey) {
    return false;
  }
  const threads = result[threadsKey];
  if (threads.length === 0) {
    return true;
  }
  return threads.some((thread) => {
    ...
    return ["cwd", "path", "title", "preview", "updatedAt", "updated_at", "updated_at_ms", "model"].some((key) => (
      Object.prototype.hasOwnProperty.call(thread, key)
    ));
  });
}
...
function sanitizeThreadHistoryImagesForRelay(rawMessage, requestMethod) {
  ...
  let thread = parsed?.result?.thread;
  ...
  const threadId = normalizeNonEmptyString(thread.id)
    || normalizeNonEmptyString(thread.threadId)
    || normalizeNonEmptyString(thread.thread_id);
  if (threadId && thread.turns.length === 0) {
    const fallbackResult = buildJsonlThreadReadFallbackResult(threadId);
    if (fallbackResult?.thread?.turns?.length > 0) {
      thread = {
        ...thread,
        ...fallbackResult.thread,
        turns: fallbackResult.thread.turns,
      };
      effectiveParsed = {
        ...parsed,
        result: {
  ...
  if (!didSanitize) {
    const trimmedPayload = trimThreadPayloadForRelay(effectiveParsed, thread);
...
function buildJsonlThreadReadFallbackResult(threadId) {
  try {
    const rolloutPath = findRecentRolloutFileForContextRead(resolveSessionsRoot(), { threadId });
    if (!rolloutPath) {
      return null;
    }
    return readThreadReadResultFromSessionJsonl(rolloutPath, {
      threadId,
      limit: RELAY_TURNS_LIST_MAX_INITIAL_LIMIT,
      maxLimit: RELAY_TURNS_LIST_MAX_INITIAL_LIMIT,
    });
  } catch (error) {
    console.warn(`[remodex] thread/read jsonl fallback failed: ${error.message}`);
    return null;
  }
}
...
function injectDesktopThreadStateForRelay(rawMessage, requestMethod) {
  if (requestMethod !== "thread/list") {
    return rawMessage;
  }

  const pinnedThreadIdList = readCodexPinnedThreadIdList();
  const pinnedThreadIds = new Set(pinnedThreadIdList);
  const pinnedThreadRanks = new Map(pinnedThreadIdList.map((id, index) => [id, index]));
  if (pinnedThreadIds.size === 0) {
    appendPinnedSyncDebugLog({ stage: "no-pins" });
    return rawMessage;
  }
  ...
  const threads = result[threadsKey].map((thread) => {
    if (!thread || typeof thread !== "object" || Array.isArray(thread)) {
      return thread;
    }
    const threadId = normalizeNonEmptyString(thread.id)
    ...
    if (!threadId || !pinnedThreadIds.has(threadId)) {
      return thread;
    }
    didChange = true;
    return {
      ...
      isPinned: true,
      pinned: true,
      pinnedRank: pinnedThreadRanks.get(threadId),
      pinnedIndex: pinnedThreadRanks.get(threadId),
    };
  });
  const syntheticPinnedThreads = buildSyntheticPinnedThreadsForRelay({
    pinnedThreadIdList,
    existingThreadIds,
    result,
  });
  const nextThreads = syntheticPinnedThreads.length > 0
    ? threads.concat(syntheticPinnedThreads)
    : threads;

  appendPinnedSyncDebugLog({
    stage: "thread-list",
    pinnedCount: pinnedThreadIds.size,
    threadCount: nextThreads.length,
    injectedCount: nextThreads.filter((thread) => thread?.isPinned === true || thread?.pinned === true).length,
    syntheticCount: syntheticPinnedThreads.length,
    samplePinnedIds: Array.from(pinnedThreadIds).slice(0, 5),
    sampleThreadIds: nextThreads.slice(0, 5).map((thread) => thread?.id || thread?.threadId || thread?.thread_id || null),
  });

  if (!didChange && syntheticPinnedThreads.length === 0) {
    return rawMessage;
  }
  ...
  return JSON.stringify({
    ...
    result: {
      ...result,
      [threadsKey]: nextThreads,
    },
  });
}

function buildSyntheticPinnedThreadsForRelay({
  pinnedThreadIdList,
  existingThreadIds,
  result,
}) {
  if (!isTerminalThreadListPage(result)) {
    return [];
  }
  const sessionIndexById = readCodexSessionIndexById();
  const sessionsRoot = resolveSessionsRoot();
  const out = [];
  for (const [pinnedRank, threadId] of pinnedThreadIdList.entries()) {
    if (existingThreadIds.has(threadId)) {
      continue;
    }
    const indexRow = sessionIndexById.get(threadId) || {};
    const rolloutPath = findRolloutFileForThread(sessionsRoot, threadId);
    const rolloutSummary = readSessionSummaryFromJsonl(rolloutPath);
    const thread = buildSyntheticPinnedThread(threadId, indexRow, rolloutSummary, pinnedRank);
    if (thread) {
      out.push(thread);
    }
  }
...
function isTerminalThreadListPage(result) {
  return !hasRelayCursor(result?.nextCursor) && !hasRelayCursor(result?.next_cursor);
}

function readCodexSessionIndexById() {
...
function readSessionSummaryFromJsonl(filePath) {
  if (!filePath) {
    return {};
  }
  let content = "";
  ...
  for (const line of content.split(/\r?\n/).slice(0, 200)) {
    ...
    if (entry?.type === "session_meta" && entry.payload && typeof entry.payload === "object") {
      summary.createdAt = normalizeNonEmptyString(entry.payload.timestamp);
      summary.cwd = normalizeNonEmptyString(entry.payload.cwd);
      summary.model = normalizeNonEmptyString(entry.payload.model);
      summary.modelProvider = normalizeNonEmptyString(entry.payload.model_provider)
        || normalizeNonEmptyString(entry.payload.modelProvider);
    } else if (!summary.preview && entry?.type === "event_msg" && entry.payload && typeof entry.payload === "object") {
      if (normalizeNonEmptyString(entry.payload.type) === "user_message") {
        summary.preview = normalizeNonEmptyString(entry.payload.message)
          || normalizeNonEmptyString(entry.payload.text);
      }
    }
    if (summary.createdAt && summary.cwd && summary.preview) {
      break;
    }
  }
  return summary;
...
function buildSyntheticPinnedThread(threadId, indexRow, rolloutSummary, pinnedRank) {
  if (Object.keys(indexRow).length === 0 && Object.keys(rolloutSummary).length === 0) {
    return null;
  }
  const title = normalizeNonEmptyString(indexRow.thread_name)
    || normalizeNonEmptyString(indexRow.title)
    || normalizeNonEmptyString(rolloutSummary.preview)
    || threadId;
  const updatedAt = normalizeNonEmptyString(indexRow.updated_at)
    || normalizeNonEmptyString(indexRow.updatedAt)
    || normalizeNonEmptyString(rolloutSummary.createdAt);
  return {
    id: threadId,
    title,
    name: title,
    preview: normalizeNonEmptyString(rolloutSummary.preview) || title,
    createdAt: normalizeNonEmptyString(rolloutSummary.createdAt) || updatedAt,
    updatedAt,
    cwd: normalizeNonEmptyString(rolloutSummary.cwd) || undefined,
    model: normalizeNonEmptyString(rolloutSummary.model) || undefined,
    modelProvider: normalizeNonEmptyString(rolloutSummary.modelProvider) || undefined,
    isPinned: true,
    pinned: true,
    pinnedRank,
    pinnedIndex: pinnedRank,
    remodexSyntheticPinned: true,
  };
}

function readCodexPinnedThreadIdList() {
  try {
    const parsed = JSON.parse(fs.readFileSync(resolveCodexGlobalStatePath(), "utf8"));
    const atomState = parsed?.["electron-persisted-atom-state"] || {};
    const pins = [
      ...readPinnedThreadIdsFromState(parsed),
      ...readPinnedThreadIdsFromState(atomState),
    ];
    return [...new Set(pins.map((id) => normalizeNonEmptyString(id)).filter(Boolean))];
  } catch {
    return [];
  }
}

function readPinnedThreadIdsFromState(state) {
  if (!state || typeof state !== "object" || Array.isArray(state)) {
    return [];
  }
  const pins = state["pinned-thread-ids"] || state.pinnedThreadIds || [];
  return Array.isArray(pins) ? pins : [];
}

function normalizePinnedThreadIds(rawValue) {
  if (!Array.isArray(rawValue)) {
    return [];
  }
  return [
    ...new Set(rawValue.map((id) => normalizeNonEmptyString(id)).filter(Boolean)),
  ];
}

function pinnedThreadSignature(pinnedThreadIds) {
  return normalizePinnedThreadIds(pinnedThreadIds).join("\0");
}

function createMobilePinnedOverride({
  pinnedThreadIds,
  previousPinnedThreadIds,
} = {}) {
  if (!Array.isArray(pinnedThreadIds)) {
  ...
  const signature = pinnedThreadSignature(normalized);
  const staleSignatures = new Set();
  if (Array.isArray(previousPinnedThreadIds)) {
    const previousSignature = pinnedThreadSignature(previousPinnedThreadIds);
    if (previousSignature !== signature) {
      staleSignatures.add(previousSignature);
    }
  }
...
function resolveMobilePinnedOverrideChange({
  pinnedThreadIds,
  override,
} = {}) {
  const normalized = normalizePinnedThreadIds(pinnedThreadIds);
  ...
  const signature = pinnedThreadSignature(normalized);
  if (signature === override.signature) {
    return {
      action: "accept-mobile",
      pinnedThreadIds: override.pinnedThreadIds,
      nextOverride: override,
    };
  }

  if (override.staleSignatures?.has(signature)) {
    return {
      action: "reassert-mobile",
      pinnedThreadIds: override.pinnedThreadIds,
      nextOverride: override,
    };
  }
...
function findThreadListResultKey(result) {
  if (!result || typeof result !== "object" || Array.isArray(result)) {
    return "";
  }
  return ["data", "items", "threads"].find((key) => Array.isArray(result[key])) || "";
}

function resolveCodexGlobalStatePath() {
  return process.env.REMODEX_CODEX_GLOBAL_STATE_PATH || DEFAULT_CODEX_GLOBAL_STATE_PATH;
}

function appendPinnedSyncDebugLog(entry) {
  if (process.env.REMODEX_PINNED_SYNC_DEBUG !== "1") {
    return;
  }
  try {
    fs.appendFileSync(
      path.join(os.tmpdir(), "remodex-pinned-sync-debug.log"),
      `${JSON.stringify({ t: new Date().toISOString(), ...entry })}\n`,
    );
  } catch {
    // Debug logging must never affect relay traffic.
  }
}

function createCodexPinnedStateWatcher({
  onChange,
  intervalMs = 1_000,
} = {}) {
  ...
  const statePath = resolveCodexGlobalStatePath();
  let lastSignature = readCodexPinnedThreadIdList().join("\0");
  const listener = () => {
    const pinnedThreadIds = readCodexPinnedThreadIdList();
    const nextSignature = pinnedThreadIds.join("\0");
    if (nextSignature === lastSignature) {
      return;
    }
    lastSignature = nextSignature;
    onChange(pinnedThreadIds);
  };

  try {
    fs.watchFile(statePath, { interval: intervalMs }, listener);
  } catch {
    return { stop() {} };
  }
  ...
  return {
    stop() {
      fs.unwatchFile(statePath, listener);
    },
  };
}
...
module.exports = {
  ...
  persistBridgePreferences,
  injectDesktopThreadStateForRelay,
  inferRelayBoundResponseMethodForDesktopState,
  sanitizeLiveGeneratedImageMessageForRelay,
  sanitizeThreadHistoryImagesForRelay,
  resolveMobilePinnedOverrideChange,
  startBridge,
};
