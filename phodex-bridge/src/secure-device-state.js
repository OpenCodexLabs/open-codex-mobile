...
// Keeps one relay session until reset-pairing, so trusted phones can reconnect across bridge restarts.
function resolveBridgeRelaySession(state, { persist = true } = {}) {
  const existingSessionId = normalizeNonEmptyString(state?.relaySessionId);
...
function normalizeBridgeDeviceState(rawState) {
  ...
  const relaySessionId = normalizeNonEmptyString(rawState?.relaySessionId) || null;
