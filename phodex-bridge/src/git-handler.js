...
function handleGitRequest(rawMessage, sendResponse, options = {}) {
  ...
  if (!method.startsWith("git/") && !["thread/generateTitle", "thread/name/set", "thread/pin/set", "thread/pins/set"].includes(method)) {
    return false;
  }
  ...
  handleGitMethod(method, params, methodOptions)
    .then((result) => {
      ...
      } else if (method === "thread/pin/set") {
        options.onThreadPinSet?.(result);
      } else if (method === "thread/pins/set") {
        options.onThreadPinsSet?.(result);
      }
    })
    .catch((err) => {
...
async function handleGitMethod(method, params, options = {}) {
  ...
  if (method === "thread/pin/set") {
    return threadPinSet(params);
  }
  if (method === "thread/pins/set") {
    return threadPinsSet(params);
  }
...
// Owns mobile pin/unpin locally so the phone and Codex Desktop share the same pinned state file.
function threadPinSet(params) {
  const threadId = normalizeNonEmptyLine(params.threadId || params.thread_id || params.conversationId || params.conversation_id);
  const pinned = typeof params.pinned === "boolean"
    ? params.pinned
    : typeof params.isPinned === "boolean"
      ? params.isPinned
      : null;
  if (!threadId) {
    throw gitError("missing_thread_id", "A thread ID is required to update pinned state.");
  }
  if (pinned == null) {
    throw gitError("missing_pinned_state", "A pinned boolean is required.");
  }

  const statePath = resolveCodexGlobalStatePath();
  const state = readCodexGlobalState(statePath);
  const previousPins = readPinnedThreadIdsFromCodexState(state);
  const nextPins = pinned
    ? [threadId, ...previousPins.filter((id) => id !== threadId)]
    : previousPins.filter((id) => id !== threadId);

  writePinnedThreadIdsToCodexState(statePath, state, nextPins);
  ...
  return {
    ...
    pinned,
    isPinned: pinned,
    pinnedRank: pinned ? nextPins.indexOf(threadId) : null,
    pinnedIndex: pinned ? nextPins.indexOf(threadId) : null,
    previousPinnedThreadIds: previousPins,
    previous_pinned_thread_ids: previousPins,
    pinnedThreadIds: nextPins,
  };
}

function threadPinsSet(params) {
  const requestedPins = normalizeThreadIdArray(
    params.pinnedThreadIds || params.pinned_thread_ids || params.threadIds || params.thread_ids
  );
  if (!Array.isArray(requestedPins)) {
    throw gitError("missing_pinned_thread_ids", "Pinned thread IDs are required.");
  }

  const statePath = resolveCodexGlobalStatePath();
  const state = readCodexGlobalState(statePath);
  const previousPins = readPinnedThreadIdsFromCodexState(state);
  const requestedSet = new Set(requestedPins);
  const nextPins = [
    ...requestedPins,
    ...previousPins.filter((id) => !requestedSet.has(id)),
  ];
  ...
  return {
    ...
    previous_pinned_thread_ids: previousPins,
    pinnedThreadIds: nextPins,
    pinned_thread_ids: nextPins,
  };
}

function writePinnedThreadIdsToCodexGlobalState(pinnedThreadIds) {
  const normalized = normalizeThreadIdArray(pinnedThreadIds);
  if (!Array.isArray(normalized)) {
    throw gitError("missing_pinned_thread_ids", "Pinned thread IDs are required.");
  }
  const statePath = resolveCodexGlobalStatePath();
  const state = readCodexGlobalState(statePath);
  writePinnedThreadIdsToCodexState(statePath, state, normalized);
  return normalized;
}
...
function normalizeThreadIdArray(rawValue) {
  if (!Array.isArray(rawValue)) {
    return null;
  }

  return [
    ...new Set(rawValue.map((item) => normalizeNonEmptyLine(item)).filter(Boolean)),
  ];
}
...
function readCodexGlobalState(statePath) {
  try {
    ...
    throw gitError("invalid_codex_global_state", "Codex global state could not be read.");
  }
}

function readPinnedThreadIdsFromCodexState(state) {
  const atomState = state?.["electron-persisted-atom-state"] || {};
  const pins = [
    ...readPinnedThreadIdsFromStateObject(state),
    ...readPinnedThreadIdsFromStateObject(atomState),
  ];
  return [...new Set(pins.map((id) => normalizeNonEmptyLine(id)).filter(Boolean))];
}

function readPinnedThreadIdsFromStateObject(state) {
...
function writePinnedThreadIdsToCodexState(statePath, state, pinnedThreadIds) {
  const nextState = state && typeof state === "object" && !Array.isArray(state) ? { ...state } : {};
  nextState["pinned-thread-ids"] = pinnedThreadIds;
